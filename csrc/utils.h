...
// tensor") #define CHECK_INPUT(x) AT_ASSERTM(x, "Input mismatch") #define
// CHECK_LT(low, high) AT_ASSERTM(low < high, "low must be smaller than high")
...
// from_vector(const phmap::flat_hash_map<key_t, std::vector<scalar_t>>
// &vec_dict,
//             bool inplace = false) {
