...
@pytest.mark.parametrize("device", places)
def test_ind2ptr(device):
    row = tensor([2, 2, 4, 5, 5, 6], paddle.int64, device)

    rowptr = paddle_sparse_ops.custom_ind2ptr(row, 8)
    assert rowptr.tolist() == [0, 0, 0, 2, 2, 3, 5, 6, 6]

    row = paddle_sparse_ops.custom_ptr2ind(rowptr, 6)
    ...
    row = paddle_sparse_ops.custom_ptr2ind(rowptr, 0)
    assert row.tolist() == []


@pytest.mark.parametrize("dtype,device", product(dtypes, places))
