...
def sample(src: SparseTensor, num_neighbors: int,
           subset: Optional[paddle.Tensor] = None) -> paddle.Tensor:
...
def sample_adj(src: SparseTensor, subset: paddle.Tensor, num_neighbors: int,
               replace: bool = False) -> Tuple[SparseTensor, paddle.Tensor]:

    rowptr, col, value = src.csr()

    rowptr, col, n_id, e_id = torch.ops.paddle_sparse.sample_adj(
