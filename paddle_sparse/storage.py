...
class SparseStorage(object):
    ...
    def __init__(
        ...
        if row is not None:
            ...
            assert row.place._equals(col.place)
            assert row.dim() == 1
        ...
        if colptr is not None:
            ...
            assert colptr.place._equals(col.place)
            assert colptr.dim() == 1
        ...
        if colcount is not None:
            ...
            assert colcount.place._equals(col.place)
        ...
        if csr2csc is not None:
            ...
            assert csr2csc.place._equals(col.place)
        ...
        if csc2csr is not None:
            ...
            assert csc2csr.place._equals(col.place)
        ...
        if not is_sorted and self._col.numel() > 0:
            idx = paddle.zeros((self._col.numel() + 1), dtype=self._col.dtype).to(
                self._col.place
            )
            ...
            if (idx[1:] < idx[:-1]).any():
                ...
                perm = index_sort(idx[1:], max_value)
    ...
    def row(self):
        ...
        rowptr = self._rowptr
        if rowptr is not None:
            row = paddle.ops.paddle_sparse.ptr2ind(rowptr, self._col.numel())
            self._row = row
            return row
    ...
    def rowptr(self) -> paddle.Tensor:
        ...
        row = self._row
        if row is not None:
            rowptr = paddle_sparse_ops.custom_ind2ptr(row, self._sparse_sizes[0])
            self._rowptr = rowptr
            return rowptr
    ...
    def set_value_(
        ...
        if value is not None:
            ...
            value = value.contiguous()
            assert value.place._equals(self._col.place)
    ...
    def sparse_resize(self, sparse_sizes: Tuple[int, int]):
        ...
        if diff_0 > 0:
            if rowptr is not None:
                rowptr = paddle.cat([rowptr, rowptr.new_full((diff_0,), nnz)])
            if rowcount is not None:
                rowcount = paddle.cat([rowcount, rowcount.new_zeros(diff_0)])
        elif diff_0 < 0:
            if rowptr is not None:
        ...
        if diff_1 > 0:
            if colptr is not None:
                colptr = paddle.cat([colptr, colptr.new_full((diff_1,), nnz)])
            if colcount is not None:
                colcount = paddle.cat([colcount, colcount.new_zeros(diff_1)])
        elif diff_1 < 0:
            if colptr is not None:
    ...
    def colptr(self) -> paddle.Tensor:
        ...
        if csr2csc is not None:
            colptr = paddle_sparse_ops.custom_ind2ptr(
                self._col[csr2csc], self._sparse_sizes[1]
            )
        else:
            colptr = paddle.zeros(
                (self._sparse_sizes[1] + 1), dtype=self._col.dtype
            ).to(self._col.place)
    ...
    def colcount(self) -> paddle.Tensor:
        ...
        if colptr is not None:
            ...
            colcount = scatter_add(
                paddle.ones_like(self._col).to(self._col.place),
                self._col,
    ...
    def csr2csc(self) -> paddle.Tensor:
        ...
        max_value = self._sparse_sizes[0] * self._sparse_sizes[1]
        csr2csc = index_sort(idx, max_value)
        self._csr2csc = csr2csc
        return csr2csc
    ...
    def csc2csr(self) -> paddle.Tensor:
        ...
        max_value = self._sparse_sizes[0] * self._sparse_sizes[1]
        csc2csr = index_sort(self.csr2csc(), max_value)
        self._csc2csr = csc2csr
        return csc2csr
    ...
    def coalesce(self, reduce: str = "add"):
        idx: paddle.Tensor = paddle.full(
            (self._col.numel() + 1,), -1, dtype=self._col.dtype
        ).to(self._col.place)
        idx[1:] = self._sparse_sizes[1] * self.row() + self._col
        mask = idx[1:] > idx[:-1]
        ...
        if value is not None:
            ...
            ptr = paddle.concat(
                [ptr, paddle.full((1), value.shape[0], dtype=ptr.dtype).to(ptr.place)]
            )
    ...
    def cuda(self):
        ...
        if new_col.place._equals(self._col.place):
            return self
