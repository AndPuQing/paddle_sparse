...
def add(src, other):  # noqa: F811
    if isinstance(other, Tensor):
        ...
        if other.shape[0] == src.shape[0] and other.shape[1] == 1:  # Row-wise.
            other = gather_csr(other.squeeze(1), rowptr)
        elif other.shape[0] == 1 and other.shape[1] == src.shape[1]:  # Col-wise.
        ...
        M = max(src.shape[0], other.shape[0])
        N = max(src.shape[1], other.shape[1])
        sparse_sizes = (M, N)
