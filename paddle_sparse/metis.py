...
def partition(
    ...
    assert num_parts >= 1
    if num_parts == 1:
        partptr = paddle.Tensor([0, src.size(0)], device=src.device())
    ...
    if balance_edge and node_weight:
        raise ValueError("Cannot set 'balance_edge' and 'node_weight' at the "
                         "same time in 'paddle_sparse.partition'")
    ...
    if node_weight is not None:
        ...
        cluster = torch.ops.paddle_sparse.partition2(rowptr, col, value,
                                                    node_weight, num_parts,
                                                    recursive)
    else:
        cluster = torch.ops.paddle_sparse.partition(rowptr, col, value,
                                                   num_parts, recursive)
    cluster = cluster.to(src.device())

    cluster, perm = cluster.sort()
    out = permute(src, perm)
    partptr = torch.ops.paddle_sparse.ind2ptr(cluster, num_parts)
